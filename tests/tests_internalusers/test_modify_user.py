...
class TestModifyUser(BaseTestCase):
    ...
    def setUp(self):
        self.user = "DummyUser"
        self.parameters = {"hash": "$2a$1234", "roles": ["DummyRole"]}
    ...
    def test_modify_user_returns_when_successfully_modified_user(self):
        self.assertIsNone(modify_user(self.user, self.parameters))
    ...
    def test_modify_user_returns_exception_when_user_does_not_exist(self):
        ...
        with self.assertRaises(ModifyUserException):
            modify_user(self.user, self.parameters)
    ...
    def test_modify_user_returns_error_when_parameter_argument_missing(self):
